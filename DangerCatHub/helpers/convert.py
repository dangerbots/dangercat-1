...
from DangerCatHub import LOGS
from DangerCatHub.utils.extras import edit_or_reply as eor
...
async def convert_to_image(event, client):
    dangercat = await event.get_reply_message()
    if not (
        dangercat.gif
        or dangercat.audio
        or dangercat.voice
        or dangercat.video
        or dangercat.video_note
        or dangercat.photo
        or dangercat.sticker
        or dangercat.media
    ):
    ...
    if not os.path.exists(downloaded_file_name):
        ...
        return
    if dangercat and dangercat.photo:
        final_danger = downloaded_file_name
    elif dangercat.sticker and dangercat.sticker.mime_type == "application/x-tgsticker":
        ...
        os.remove(rpath)
        final_danger = image_name20
    elif dangercat.sticker and dangercat.sticker.mime_type == "image/webp":
        ...
        if not os.path.exists(image_new_path):
            ...
            return
        final_danger = image_new_path
    elif dangercat.audio:
        ...
        if not os.path.exists(imgpath):
            await eor(event, "`Unable To Fetch Shot.`")
            return
        final_danger = imgpath
    elif dangercat.gif or dangercat.video or dangercat.video_note:
