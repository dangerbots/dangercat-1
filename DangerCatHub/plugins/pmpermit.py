...
from DangerCatHub.DB import pmpermit_sql as pm_sql
...
@danger_cat_cmd(pattern="unblock$")
...
if PM_ON_OFF and PM_ON_OFF not in PM_PARAMS:
    ...
    @danger_cat_cmd(pattern="(a|approve|allow)$")
    ...
    @danger_cat_cmd(pattern="(da|disapprove|disallow)$")
    ...
    @danger_cat_cmd(pattern="listapproved$")
    ...
    async def new_pm(event):
        if not event.is_private:
            return
        DANGERCATUSERID, _, dangerbot_mention = await client_id(event)
        ...
        if CSTM_PMP:
             HELL_FIRST += f"\n\n{CSTM_PMP}"
        if event.sender_id == DANGERCATUSERID:
            return
