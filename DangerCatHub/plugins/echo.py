...
from DangerCatHub.DB.echo_sql import addecho, get_all_echos, is_echo, remove_echo
from DangerCatHub.DB.gvar_sql import addgvar, gvarstat

from . import *


@danger_cat_cmd(pattern="echo$")
...
@danger_cat_cmd(pattern="listecho$")
async def echo(event):
    lsts = get_all_echos()
    if len(lsts) > 0:
