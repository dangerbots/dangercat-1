...
@danger_cat_cmd(pattern="wiki(?:\s|$)([\s\S]*)")
async def _(event):
    match = event.text[6:]
    result = None
    try:
...
@danger_cat_cmd(pattern="google(?:\s|$)([\s\S]*)")
...
@danger_cat_cmd(pattern="img(?:\s|$)([\s\S]*)")
async def img(event):
...
@danger_cat_cmd(pattern="reverse(?:\s|$)([\s\S]*)")
async def _(event):
...
@danger_cat_cmd(pattern="gps(?:\s|$)([\s\S]*)")
async def gps(event):
...
@danger_cat_cmd(pattern="webshot ([\s\S]*)")
async def _(event):
...
@danger_cat_cmd(pattern="cricket$")
async def _(event):
