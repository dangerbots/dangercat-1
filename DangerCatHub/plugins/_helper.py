...
async def repo(event):
    ...
    try:
        ...
        if event.sender_id == madinferno:
            await event.delete()
    except (noin, dedbot):
        await eor(event, msg)


@danger_cat_cmd(pattern="help$")
async def _(event):
    tgbotusername = Config.BOT_USERNAME
    chat = "@Botfather"
    if tgbotusername is not None:
        try:
            results = await event.client.inline_query(tgbotusername, "dangerbot_help")
...
@danger_cat_cmd(pattern="plinfo(?:\s|$)([\s\S]*)")
async def dangerbot(event):
...
@danger_cat_cmd(pattern="cmdinfo(?:\s|$)([\s\S]*)")
