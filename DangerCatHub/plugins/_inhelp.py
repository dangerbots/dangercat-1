...
dangerbot_row = Config.BUTTONS_IN_HELP
dangerbot_emoji = Config.EMOJI_IN_HELP
        ...
        buttons.append(
            [
                custom.Button.inline(
                    f"{dangerbot_emoji} " + pair + f" {dangerbot_emoji}",
...
if Config.BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        DANGERCATUSERID, DANGERCATUSER, dangerbot_mention = cids[0], cids[1], cids[2]
        builder = event.builder
        result = None
        query = event.text
        auth = await clients_list()
        if event.query.user_id in auth and query == "dangerbot_help":
            rev_text = query[::-1]
            ...
            alive_msg = gvarstat("ALIVE_MSG") or "»»» <b>DangerCat is Alive</b> «««"
            alive_name = gvarstat("ALIVE_NAME") or DANGERCATUSER
            he_ll = alive_txt.format(
                alive_msg, tel_ver, hell_ver, uptime, abuse_m, is_sudo
            )
            alv_btn = [
                [Button.url(f"{alive_name}", f"tg://openmessage?user_id={DANGERCATUSERID}")],
                [
            ...
            HELL_FIRST = f"🔥 𝙃𝙚𝙡𝙡𝘽𝙤𝙩 𝙋𝙈 𝙎𝙚𝙘𝙪𝙧𝙞𝙩𝙮 🔥\n\nHello!! This is an automated message on behalf of {dangerbot_mention}."
            if CSTM_PMP:
                HELL_FIRST += f"\n\n{CSTM_PMP}"
            ...
            result = builder.article(
                ...
                buttons=[
                    ...
                    [Button.url("DangerCatNetwork", "https://t.me/hellbot_network")],
                ],
            )
    ...
    async def on_plug_in_callback_query_handler(event):
        ...
        if event.query.user_id in auth:
            veriler = custom.Button.inline(
                f"{dangerbot_emoji} Re-Open Menu {dangerbot_emoji}", data="reopen"
            )
            await event.edit(
                f"**⚜️ Hêllẞø† Mêñû Prõvîdêr ìs ñôw Çlösëd ⚜️**\n\n**Bot Of :**  {dangerbot_mention}\n\n        [©️ Hêllẞø† ™️]({chnl_link})",
                buttons=veriler,
                link_preview=False,
            )
        else:
    ...
    async def send(event):
        ...
        DANGERCATUSERID, DANGERCATUSER, dangerbot_mention = await client_id(event, event.query.user_id)
        thumb = dangerbot_logo
        omk = f"**• Plugin name ≈** `{plugin}`\n**• Uploaded by ≈** {dangerbot_mention}\n\n⚡ **[ʟɛɢɛռɖaʀʏ ᴀғ ɦɛʟʟɮօt]({chnl_link})** ⚡"
        the_plugin_file = "./DangerCatHub/plugins/{}.py".format(plugin.lower())
        butt = custom.Button.inline(f"{dangerbot_emoji} Main Menu {dangerbot_emoji}", data="reopen")
    ...
    async def Information(event):
        ...
        buttons.append([custom.Button.inline(f"{dangerbot_emoji} Main Menu {dangerbot_emoji}", data=f"page({page})")])
        if event.query.user_id in auth:
            await event.edit(
    ...
    async def commands(event):
        ...
        if event.query.user_id in auth:
            await event.edit(
                ...
                buttons=[
                    custom.Button.inline(
                        f"{dangerbot_emoji} Return {dangerbot_emoji}",
