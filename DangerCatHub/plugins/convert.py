...
@danger_cat_cmd(pattern="stog(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    if type_ == 'Sticker':
        ...
        await hell.delete()
        os.remove(hell_)
        os.remove("dangerbot.gif")
    else:
        await parse_error(hell, "Only animated stickers are supported.")


@danger_cat_cmd(pattern="stoi$")
async def _(dangerCat):
    reply_to_id = dangerCat.message.id
    if dangerCat.reply_to_msg_id:
        reply_to_id = dangerCat.reply_to_msg_id
    event = await eor(dangerCat, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        downloaded_file_name = await dangerCat.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            await dangerCat.client.send_file(
                event.chat_id,
...
@danger_cat_cmd(pattern="ttf(?:\s|$)([\s\S]*)")
async def get(event):
    name = event.text[5:]
    if name is None:
...
@danger_cat_cmd(pattern="nfc(?:\s|$)([\s\S]*)")
async def _(event):
    if not event.reply_to_msg_id:
