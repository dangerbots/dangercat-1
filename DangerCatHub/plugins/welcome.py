...
from DangerCatHub.DB.gvar_sql import addgvar, delgvar, gvarstat
from DangerCatHub.DB.welcome_sql import (add_welcome, get_current_welcome,
...
async def _(event):
    ...
    if not gvarstat(f"WELCOME_{DANGERCATUSERID}_{str(event.chat_id)[1:]}"):
        return
...
@danger_cat_cmd(pattern="savewelcome(?:\s|$)([\s\S]*)")
async def save_welcome(event):
    ...
    if add_welcome(event.chat_id, 0, string, msg_id) is True:
        addgvar(f"WELCOME_{DANGERCATUSERID}_{str(event.chat_id)[1:]}", "TRUE")
        return await eor(event, success.format("saved"))
    rm_welcome(event.chat_id)
    delgvar(f"WELCOME_{DANGERCATUSERID}_{str(event.chat_id)[1:]}")
    if add_welcome(event.chat_id, 0, string, msg_id) is True:
        addgvar(f"WELCOME_{DANGERCATUSERID}_{str(event.chat_id)[1:]}", "TRUE")
...
@danger_cat_cmd(pattern="cleanwelcome$")
async def del_welcome(event):
    DANGERCATUSERID, DANGERCATUSER, dangerbot_mention = await client_id(event)
    if gvarstat(f"WELCOME_{DANGERCATUSERID}_{str(event.chat_id)[1:]}"):
...
@danger_cat_cmd(pattern="showwelcome$")
...
@danger_cat_cmd(pattern="welcome_note$")
