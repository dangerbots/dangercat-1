...
@danger_cat_cmd(pattern="autoname$")
async def _(event):
    hell = await eor(event, "`Starting AutoName Please Wait`")
    _id, DANGERCATUSER, _ment = await client_id(event)
...
@danger_cat_cmd(pattern="autobio$")
async def _(event):
...
@danger_cat_cmd(pattern="reserved$")
