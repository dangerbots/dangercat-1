...
from DangerCatHub.DB.mute_sql import is_muted, mute, unmute
...
@danger_cat_cmd(pattern="setgpic$")
@errors_handler
...
@danger_cat_cmd(pattern="promote(?:\s|$)([\s\S]*)")
@errors_handler
async def promote(event):
    ...
    dangercat_event = await eor(event, "`Promoting User...`")
...
@danger_cat_cmd(pattern="demote(?:\s|$)([\s\S]*)")
@errors_handler
...
@danger_cat_cmd(pattern="mute(?:\s|$)([\s\S]*)")
async def muth(event):
    ...
    if event.is_private:
        ...
        if event.chat_id == DANGERCATUSERID:
        ...
        if DANGERCATUSERID not in admin_:
...
@danger_cat_cmd(pattern="unmute(?:\s|$)([\s\S]*)")
...
@danger_cat_cmd(pattern="ban(?:\s|$)([\s\S]*)")
@errors_handler
async def ban(event):
...
@danger_cat_cmd(pattern="unban(?:\s|$)([\s\S]*)")
@errors_handler
...
@danger_cat_cmd(pattern="pin(?:\s|$)([\s\S]*)")
@errors_handler
async def pin(event):
    chat = await event.get_chat()
...
@danger_cat_cmd(pattern="unpin(?:\s|$)([\s\S]*)")
...
@danger_cat_cmd(pattern="kick(?:\s|$)([\s\S]*)")
@errors_handler
...
@danger_cat_cmd(pattern="zombies(?:\s|$)([\s\S]*)")
