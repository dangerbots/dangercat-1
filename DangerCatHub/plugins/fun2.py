...
@danger_cat_cmd(pattern="pat ([\s\S]*)")
async def _(event):
...
@danger_cat_cmd(pattern="session$")
async def _(event):
    mentions = "**telethon.errors.rpcerrorlist.AuthKeyDuplicatedError: The authorization key (session file) was used under two different IP addresses simultaneously, and can no longer be used. Use the same session exclusively, or use different sessions (caused by GetMessagesRequest)**"
    await eor(event, mentions)


@danger_cat_cmd(pattern="ftext ([\s\S]*)")
...
@danger_cat_cmd(pattern="cat$")
async def hmm(hell):
    reactcat = nekos.textcat()
    await eor(hell, reactcat)


@danger_cat_cmd(pattern="why$")
async def hmm(hell):
    whyhell = nekos.why()
    await eor(hell, whyhell)


@danger_cat_cmd(pattern="fact$")
async def hmm(hell):
    facthell = nekos.fact()
