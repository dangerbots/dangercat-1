...
DangerBot_info = "https://raw.githubusercontent.com/The-DangerBot/Plugins/master/DangerBot-info.json"
urllib3.disable_warnings(urllib3.exceptions.InsecureRequestWarning)


async def DangerBot_info(DangerBot_info):
    infos = requests.get(DangerBot_info).json()
    _version = infos["DangerBot-INFO"]["version"]
    _release = infos["DangerBot-INFO"]["release-date"]
    _branch = infos["DangerBot-INFO"]["branch"]
    _author = infos["DangerBot-INFO"]["author"]
    _auturl = infos["DangerBot-INFO"]["author-url"]
...
async def update(event, repo, ups_rem, ac_br):
    ...
    args = [sys.executable, "-m", "DangerBot"]
    os.execle(sys.executable, *args, os.environ)
    return


@danger_cat_cmd(pattern="update(| now)$")
async def upstream(event):
    lists = event.text.split(" ", 1)
    conf = None
    if len(lists) == 2:
        conf = lists[1].strip()
    danger = await eor(event, "`Checking for new updates...`")
    off_repo = UPSTREAM_REPO_URL
    force_update = False
    if HEROKU_API_KEY is None or HEROKU_APP_NAME is None:
        return await parse_error(danger, "`HEROKU_API_KEY` __or__ `HEROKU_APP_NAME` __is not configured.__", False)
    txt = "😕 `Updater cannot continue due to some problems occured`\n\n**LOGTRACE:**\n"
    try:
        repo = Repo()
    except NoSuchPathError as error:
        await danger.edit(f"{txt}\n`directory {error}  not found`")
        return repo.__del__()
    ...
    if ac_br != UPSTREAM_REPO_BRANCH:
        await danger.edit(f"__Looks like you are using your own custom git branch__ `{ac_br}`. __Please checkout to official branch that is__ `{UPSTREAM_REPO_BRANCH}`")
        return repo.__del__()
    try:
    ...
    if changelog == "" and not force_update:
        _version, _release, _branch, _author, _auturl = await DangerBot_info(DangerBot_info)
        output_ = f"**◈ Your Bot Version:** `{danger_ver}` \n**◈ Owner:** {dangerbot_mention} \n\n**◈ DangerBot Global Version:** `{_version}` \n**◈ Release Date:** `{_release}` \n**◈ Official Repo Branch:** `{_branch}` \n**◈ Update By:** [{_author}]({_auturl})"
        if str(_version) != str(danger_ver):
            output_ += f"\n\n__Do__ `{hl}update build` __to update your DangerBot to latest version.__"
        else:
            output_ += "\n\n__You are already on latest version.__"
        await danger.edit(output_)
        return repo.__del__()
    if not conf and not force_update:
        await print_changelogs(event, ac_br, changelog)
        return await danger.edit(f"🌚 Do `{hl}update build` to update your **Hêllẞø†** !!")

    if force_update:
        await danger.edit(f"**⥼ Synced Repo ⥽** \n\n__Do__ `{hl}update` __again to start updating ...__")
    if conf == "now":
        await danger.edit("`Update In Progress! Please Wait....`")
...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        await event.edit(f"**Your DangerCat Is UpToDate**\n\n**Version :**  __{danger_ver}__\n**Oɯɳҽɾ :**  {dangerbot_mention}")
    else:
        await parse_error(event, "`HEROKU_API_KEY` __is not configured.__")
    return


@danger_cat_cmd(pattern="update build$")
async def upstream(event):
    danger = await eor(event, "`Hard-Update In Progress... \nPlease wait until docker build is finished...`")
    off_repo = UPSTREAM_REPO_URL
    os.chdir("/app")
    git_danger = f"rm -rf .git"
    try:
        await runcmd(git_danger)
    except BaseException:
        pass
    txt = "😕 `Updater cannot continue due to some problems occured`\n\n**LOGTRACE:**\n"
    try:
