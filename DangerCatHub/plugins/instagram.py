...
@danger_cat_cmd(pattern="igdl(?:\s|$)([\s\S]*)")
                    ...
                    for single in items_list:
                        try:
                            await event.client.send_file(
                                event.chat_id, 
                                single,
                                caption=f"📥 InstaGram Post Downloaded By :- {dangerbot_mention}",
                            )
                            count += 1
...
@danger_cat_cmd(pattern="igup(?:\s|$)([\s\S]*)")
...
@danger_cat_cmd(pattern="iguser(?:\s|$)([\s\S]*)")
