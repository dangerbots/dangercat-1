...
from DangerCatHub.DB.gvar_sql import addgvar, gvarstat
from . import *
...
@danger_cat_cmd(pattern="kang(?:\s|$)([\s\S]*)")
async def kang(event):
    ...
    DANGERCATUSERID, DANGERCATUSER, dangerbot_mention = await client_id(event)
    nick = f"@{user.username}" if user.username else DANGERCATUSER
    name = user.username if user.username else DANGERCATUSERID
    ...
    await tbot.send_message(
        Config.LOGGER_ID,
        f"#KANG #STICKER \n\nA sticker has been kanged into the pack of {DANGERCATUSER}. Click below to see the pack!",
        buttons=[
          [Button.url("View Pack", f"t.me/addstickers/{packname}")],
          [Button.url(DANGERCATUSER, f"tg://user?id={DANGERCATUSERID}")],
        ],
        parse_mode=None,
    )
    await eod(
...
@danger_cat_cmd(pattern="pkang(?:\s|$)([\s\S]*)")
async def _(event):
    ...
    user = await event.client.get_me()
    un = f"@{user.username}" if user.username else DANGERCATUSER
    un_ = user.username if user.username else DANGERCATUSERID
    if not reply:
    ...
    if reply and reply.media and reply.media.document.mime_type == "image/webp":
        ...
        await tbot.send_message(
            Config.LOGGER_ID,
            f"#PKANG #STICKER \n\nA sticker pack has been kanged by {DANGERCATUSER}. Click below to see the pack!",
            buttons=[
...
@danger_cat_cmd(pattern="stkrinfo$")
...
@danger_cat_cmd(pattern="delst(?:\s|$)([\s\S]*)")
async def _(event):
    reply = await event.get_reply_message()
    if not reply:
...
@danger_cat_cmd(pattern="editst(?:\s|$)([\s\S]*)")
async def _(event):
    reply = await event.get_reply_message()
    if not reply:
...
@danger_cat_cmd(pattern="text(?:\s|$)([\s\S]*)")
...
@danger_cat_cmd(pattern="waifu(?:\s|$)([\s\S]*)")
