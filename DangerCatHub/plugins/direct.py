...
@danger_cat_cmd(pattern="direct(?:\s|$)([\s\S]*)")
async def direct_link_generator(request):
    if request.fwd_from:
        return
    dangercat_event = await eor(request, "`Processing...`")
    ...
    if message:
        ...
        await dangercat_event.edit(f"**Usage :** {hl}direct <url>`")
        return
    reply = ""
    links = re.findall(r"\bhttps?://.*\.\S+", message)
    if not links:
        reply = "`No links found!`"
        await dangercat_event.edit(reply)
    for link in links:
