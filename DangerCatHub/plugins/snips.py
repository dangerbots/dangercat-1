from DangerCatHub.DB import snip_sql as sq

from . import *


@danger_cat_cmd(pattern=r"\#(\S+)")
...
@danger_cat_cmd(pattern="snip(?:\s|$)([\s\S]*)")
...
@danger_cat_cmd(pattern="rmsnip(?:\s|$)([\s\S]*)")
async def _(event):
    Config.LOGGER_ID
...
@danger_cat_cmd(pattern="listsnip$")
