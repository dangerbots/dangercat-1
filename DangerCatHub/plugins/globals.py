...
from DangerCatHub.DB import gmute_sql as gsql
from DangerCatHub.DB.gban_sql import all_gbanned, gbaner, is_gbanned, ungbaner
...
@danger_cat_cmd(pattern="gpro(?:\s|$)([\s\S]*)")
async def _(event):
...
@danger_cat_cmd(pattern="gdem(?:\s|$)([\s\S]*)")
async def _(event):
    DANGERCATUSERID, DANGERCATUSER, dangerbot_mention = await client_id(event)
...
@danger_cat_cmd(pattern="gban(?:\s|$)([\s\S]*)")
async def _(event):
    reason = ""
    DANGERCATUSERID, DANGERCATUSER, dangerbot_mention = await client_id(event)
    ...
    gmsg = f"🥴 [{name}](tg://user?id={userid}) **beta majdur ko khodna 😪 aur** {dangerbot_mention} **ko chodna... Kabhi sikhana nhi!! 😏**\n\n📍 Added to Gban Watch!!\n**🔰 Total Chats :**  `{chats}`"
    if reason != "":
        gmsg += f"\n**🔰 Reason :**  `{reason}`"
    
    ogmsg = f"**📍 Victim:** [{name}](tg://user?id={userid}) \n**📍 Chats:** `{chats}` \n**📍 Gban By:** {dangerbot_mention}\n\n**📍 User Added to Gban Watch!!**"
    if reason != "":
        ogmsg += f"\n**📍 Reason:** `{reason}`"
...
@danger_cat_cmd(pattern="ungban(?:\s|$)([\s\S]*)")
async def _(event):
    DANGERCATUSERID, DANGERCATUSER, dangerbot_mention = await client_id(event)
...
@danger_cat_cmd(pattern="listgban$")
...
@danger_cat_cmd(pattern="gkick(?:\s|$)([\s\S]*)")
...
@danger_cat_cmd(pattern="gmute(?:\s|$)([\s\S]*)")
async def gm(event):
    DANGERCATUSERID, DANGERCATUSER, dangerbot_mention = await client_id(event)
...
@danger_cat_cmd(pattern="ungmute(?:\s|$)([\s\S]*)")
...
@dangerbot_handler(incoming=True)
