...
from DangerCatHub.DB.filter_sql import add_filter, get_all_filters, remove_all_filters, remove_filter
...
async def on_snip(event):
    ...
    if not gvarstat(f"FILTER_{DANGERCATUSERID}_{str(_id)[1:]}"):
        return
...
@danger_cat_cmd(pattern="filter(?:\s|$)([\s\S]*)")
async def on_snip_save(event):
    name = event.pattern_match.group(1)
    msg = await event.get_reply_message()
    ForGo10God, _, dangerbot_mention = await client_id(event)
    _id = event.chat_id
    if msg:
...
@danger_cat_cmd(pattern="filters$")
...
@danger_cat_cmd(pattern="stop(?:\s|$)([\s\S]*)")
...
@danger_cat_cmd(pattern="rmallfilters$")
