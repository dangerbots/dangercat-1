...
@danger_cat_cmd(pattern="offline$")
async def _(event):
    DANGERCATUSERID, DANGERCATUSER, dangerbot_mention = await client_id(event)
    user = await event.client.get_entity(DANGERCATUSERID)
    if DANGERCATUSER.startswith("[ • OFFLINE • ]"):
...
@danger_cat_cmd(pattern="pbio(?:\s|$)([\s\S]*)")
async def _(event):
...
@danger_cat_cmd(pattern="pname(?:\s|$)([\s\S]*)")
async def _(event):
    lists = event.text.split(" ", 1)
    if len(lists) != 2:
...
@danger_cat_cmd(pattern="username(?:\s|$)([\s\S]*)")
...
@danger_cat_cmd(pattern="myusernames$")
async def _(event):
    _, _, dangerbot_mention = await client_id(event)
    result = await event.client(GetAdminedPublicChannelsRequest())
    output_str = f"**Usernames reserved by:** {dangerbot_mention}\n\n"
