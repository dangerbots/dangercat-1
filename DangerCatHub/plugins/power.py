...
@danger_cat_cmd(pattern="restart$")
async def re(hell):
...
@danger_cat_cmd(pattern="reload$")
async def rel(event):
    await eor(event, "**Reloaded DangerBot!** \n\n__This might take a minute.__")
    await reload_DangerBot()


@danger_cat_cmd(pattern="shutdown$")
...
@danger_cat_cmd(pattern="svar(?:\s|$)([\s\S]*)")
...
@danger_cat_cmd(pattern="gvar(?:\s|$)([\s\S]*)")
...
@danger_cat_cmd(pattern="dvar(?:\s|$)([\s\S]*)")
