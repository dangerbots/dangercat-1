...
from DangerCatHub.DB.husb_sql import add_hus_grp, get_all_hus_grp, is_husb, rm_hus_grp
from DangerCatHub.DB.waifu_sql import add_grp, get_all_grp, is_harem, rm_grp
...
@danger_cat_cmd(pattern="pt(?:\s|$)([\s\S]*)")
async def _(event):
...
@danger_cat_cmd(pattern="adwaifu(?:\s|$)([\s\S]*)")
async def _(event):
...
@danger_cat_cmd(pattern="rmwaifu(?:\s|$)([\s\S]*)")
async def _(event):
    _, _, dangerbot_mention = await client_id(event)
    if not event.is_group:
        return await parse_error(event, "Autowaifu works in groups only !!")
...
@danger_cat_cmd(pattern="rmhusb(?:\s|$)([\s\S]*)")
async def _(event):
    _, _, dangerbot_mention = await client_id(event)
    if not event.is_group:
        return await parse_error(event, "Autohusbando works in groups only !!")
...
@danger_cat_cmd(pattern="autowaifu$")
async def _(event):
...
@danger_cat_cmd(pattern="autohusbando$")
async def _(event):
