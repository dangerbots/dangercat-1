from DangerCatHub.DB.autopost_sql import add_post, get_all_post, is_post, remove_post
from DangerCatHub.DB.gvar_sql import addgvar, delgvar, gvarstat

from . import *


@danger_cat_cmd(pattern="autopost(?:\s|$)([\s\S]*)")
async def _(event):
    if event.is_private:
    ...
    DANGERCATUSERID = cid[0]
    hel_ = event.text[10:]
    cli_ = DANGERCATUSERID
...
@danger_cat_cmd(pattern="rmautopost(?:\s|$)([\s\S]*)")
async def _(event):
    if event.is_private:
