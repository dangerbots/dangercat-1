...
async def pong(event):
    ...
    DANGERCATUSERID, DANGERCATUSER, _ = await client_id(event)
    dangerbot_mention = f"<a href='tg://user?id={DANGERCATUSERID}'>{DANGERCATUSER}</a>"
    ...
    if PIC:
        await event.client.send_file(
            event.chat_id,
            file=PIC,
            caption=ping_txt.format(ms, uptime, dangerbot_mention),
            parse_mode="HTML",
        )
        await hell.delete()
    else:
        await hell.edit(ping_txt.format(ms, uptime, dangerbot_mention), parse_mode="HTML")


@danger_cat_cmd(pattern="limits$")
...
@danger_cat_cmd(pattern="dc$")
async def _(event):
    result = await event.client(functions.help.GetNearestDcRequest())
    await eor(event, result.stringify())


@danger_cat_cmd(pattern="config$")
async def _(event):
...
@danger_cat_cmd(pattern="vars(?:\s|$)([\s\S]*)")
async def lst(event):
...
@danger_cat_cmd(pattern="schd(?:\s|$)([\s\S]*)")
async def _(event):
...
@danger_cat_cmd(pattern="dm(?:\s|$)([\s\S]*)")
async def _(event):
