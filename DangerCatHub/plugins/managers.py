...
@danger_cat_cmd(pattern="ls(?:\s|$)([\s\S]*)")
async def lst(event):
...
@danger_cat_cmd(pattern="ls_local$")
async def _(event):
...
@danger_cat_cmd(pattern="ls_root$")
async def _(event):
    PROCESS_RUN_TIME = 100
    cmd = "ls -lh"
...
@danger_cat_cmd(pattern="ls_saved$")
async def _(event):
    PROCESS_RUN_TIME = 100
    cmd = "ls ./SAVED/"
    reply_to_id = event.message.id
...
@danger_cat_cmd(pattern="rnsaved(?:\s|$)([\s\S]*)")
async def _(event):
    PROCESS_RUN_TIME = 100
    input_str = event.pattern_match.group(1)
    if "|" in input_str:
...
@danger_cat_cmd(pattern="rnlocal(?:\s|$)([\s\S]*)")
async def _(event):
    PROCESS_RUN_TIME = 100
    input_str = event.pattern_match.group(1)
    if "|" in input_str:
...
@danger_cat_cmd(pattern="delsave(?:\s|$)([\s\S]*)")
...
@danger_cat_cmd(pattern="delocal(?:\s|$)([\s\S]*)")
async def handler(event):
    input_str = event.pattern_match.group(1)
