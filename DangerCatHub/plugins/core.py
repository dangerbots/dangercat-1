...
@danger_cat_cmd(pattern="cmds$")
async def kk(event):
    event.message.id
    if event.reply_to_msg_id:
        event.reply_to_msg_id
    cids = await client_id(event)
    ForGo10God, DANGERCATUSER, dangerbot_mention = cids[0], cids[1], cids[2]
    cmd = "ls DangerCatHub/plugins"
...
@danger_cat_cmd(pattern="send ([\s\S]*)")
async def send(event):
    ...
    the_plugin_file = "./DangerCatHub/plugins/{}.py".format(input_str.lower())
...
@danger_cat_cmd(pattern="install(?:\s|$)([\s\S]*)")
        ...
        try:
            downloaded_file_name = (
                await event.client.download_media(  # pylint:disable=E0602
                    ...
                    "./DangerCatHub/plugins/",  # pylint:disable=E0602
                )
            )
            if owo != "-f":
...
@danger_cat_cmd(pattern="uninstall ([\s\S]*)")
async def uninstall(event):
    ...
    dir_path = f"./DangerCatHub/plugins/{shortname}.py"
    try:
...
@danger_cat_cmd(pattern="unload ([\s\S]*)")
...
@danger_cat_cmd(pattern="load ([\s\S]*)")
