...
async def _(event):
    ...
    imagePath = img
    maskPath = "./DangerCat_config/resources/pics/mask (1).png"
    cascPath = "./DangerCat_config/resources/xmls/haarcascade_frontalface_default.xml"
...
@danger_cat_cmd(pattern="trigger$")
async def _(event):
...
@danger_cat_cmd(pattern="ytc(?:\s|$)([\s\S]*)")
async def _(event):
...
@danger_cat_cmd(pattern="audiomeme(?:\s|$)([\s\S]*)")
async def _(event):
...
@danger_cat_cmd(pattern="doge(?:\s|$)([\s\S]*)")
...
@danger_cat_cmd(pattern="glax(?:\s|$)([\s\S]*)")
async def nope(event):
    lists = event.text.split(" ", 1)
    reply = await event.get_reply_message()
    query = None
...
@danger_cat_cmd(pattern="honka(?:\s|$)([\s\S]*)")
async def nope(event):
    lists = event.text.split(" ", 1)
    reply = await event.get_reply_message()
    query = None
...
@danger_cat_cmd(pattern="gogl(?:\s|$)([\s\S]*)")
async def nope(event):
    lists = event.text.split(" ", 1)
    reply = await event.get_reply_message()
    query = None
