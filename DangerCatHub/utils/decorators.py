...
from DangerCatHub import bot
from DangerCatHub.clients import H2, H3, H4, H5
...
def admin_cmd(pattern=None, command=None, **args):
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.HANDLER) == 2:
                dangerreg = "^" + Config.HANDLER
                reg = Config.HANDLER[1]
            elif len(Config.HANDLER) == 1:
                dangerreg = "^\\" + Config.HANDLER
                reg = Config.HANDLER
            args["pattern"] = re.compile(dangerreg + pattern)
    ...
    # blacklisted chats.
    # dangerbot will not respond in these chats.
